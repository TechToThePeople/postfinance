...
class CRM_Postfinance_IPN extends CRM_Core_Payment_BaseIPN {
  ...
  function main($params) {
    ...
    $this->contributionIsPaid($params['orderID']);
  }

  protected function loadContribution($contribution_id) {
    $api_result = civicrm_api('contribution', 'get', array(
      'version' => 3,
      'contribution_id' => $contribution_id,
      'is_test' => 1,
    ));
    $n = count($api_result['values']);
    print "\ncontributions for $contribution_id: $n\n";
    dpm($api_result);
  }

  protected function contributionIsPaid($contribution_id) {
    $contribution = $this->loadContribution($contribution_id);
